...
export const OcwBeeezoMultipleWinners = () => {
  ...
  const { data: currentRafflePrize } = useReadContract({
    abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
    address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
    functionName: "getCurrentRafflePrize",
  });
  ...
  const { data: currentPlayersCount, refetch: refetchPlayersCount } =
    useReadContract({
      abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
      address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
  ...
  const { data: numberOfWinnersForCurrentRaffle } = useReadContract({
    abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
    address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
    functionName: "getNumberOfWinnersForCurrentRaffle",
  });

  const { data: maxParticipants } = useReadContract({
    abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
    address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
    functionName: "getMaxParticipants",
  });

  const { data: remainingSlots } = useReadContract({
    abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
    address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
    functionName: "getRemainingSlots",
  });
  ...
  const { data: isMaxParticipantsReached } = useReadContract({
    abi: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ABI,
    address: BASE_MULTIPLE_WINNER_FREE_TIMER_CONTRACT_ADDRESS,
  ...
  const currentPrize = currentRafflePrize
    ? formatEther(currentRafflePrize as any)
    : "0";
  const ticketsSold = ticketsSoldThisRound ? Number(ticketsSoldThisRound) : 0;
  const playersCount = currentPlayersCount ? Number(currentPlayersCount) : 0;
  const totalWinners = numberOfWinnersForCurrentRaffle
    ? Number(numberOfWinnersForCurrentRaffle)
    : 0;
  const maxSlots = maxParticipants ? Number(maxParticipants) : 0;
  const slotsRemaining = remainingSlots ? Number(remainingSlots) : 0;
  ...
  const calculateChance: number =
    totalWinners > 0 && playersCount > 0
      ? Math.min(((perTicketNumber * totalWinners) / playersCount) * 100, 100)
      : 0;
  ...
  return (
    <div className="min-h-screen px-4 py-12 text-zinc-50">
      <div className="flex flex-col items-center gap-2 my-4 text-center justify-center">
        ...
        <h2 className="text-sm text-orange-400 font-bold">
          {currentPrize} ETH Prize Pool [{maxSlots} Max Participants]
        </h2>
      </div>
      <Link
      ...
      <motion.div
          ...
          <div className="text-lg text-orange-400">
            {!raffleStatus && (getRemainingTime as number) < 1 ? (
              ...
              <div className="text-orange-300">
                {!!isMaxParticipantsReached && "Max Participants Reached"}
              </div>
            )}
          </div>
        </Block>
        ...
        <Block className="col-span-5 md:col-span-5 flex flex-col items-center text-center justify-center">
          ...
          <span className="">
            {totalWinners > 0 && playersCount > 0 ? "Chance to win" : ""}
          </span>
