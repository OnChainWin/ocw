...
export async function getBaseDatafromdbwinner() {
  // send post request to /api/query
  const response = await fetch("/api/query", {
    method: "POST",
    headers: {
...
export const OCWPaidBase = () => {
  ...
  const OCWBaseEvents = async () => {
    let playersPaid: {
      player: any;
      contractType: any;
      numberOfEntries: any;
    }[] = [];
    const playersfromdb = await getBaseDatafromdb();
  ...
  const OCWWBaseEvents = async () => {
    ...
    const winnersfromdb = await getBaseDatafromdbwinner();
