...
import { formatEther, maxUint256, parseAbiItem, parseEther } from "viem";
import {
  useAccount,
  useReadContract,
...
export const OCWTokenPaid = () => {
  ...
  const OCWWEvents = async () => {
    ...
    //   prizeAmount: logz.args.targetPrizeAmount?.toString(),
    //   rewardToken: logz.args.rewardToken?.toString(),
    // }));
  ...
  const { data: allowanceUSDC } = useReadContract({
    ...
    args: [address, TOKEN_PAIDTIMER_CONTRACT_ADDRESS],
  });
  ...
  const buyTicket = async () => {
    ...
    // console.log("amountCostOnSubmit:", amountCostOnSubmit);
  ...
  const buyTicketUSDC = async () => {
    ...
    if ((!allowanceUSDC as any) > 0) {
      writeContract({
        ...
        args: [TOKEN_PAIDTIMER_CONTRACT_ADDRESS, maxUint256],
      });
    }
  ...
  const buyTicketUSDT = async () => {
    // if (amountCostUSDTOnSubmit > (balanceUSDT as any)) {
    //   toast({
      ...
      <motion.div
        ...
        <Block className="col-span-12 row-span-2 md:col-span-6 justify-between flex flex-col">
          <div className="text-xl font-bold justify-between flex flex-row gap-5">
            ...
            <Link
              href={`https://scrollscan.com/token/${getRewardToken}`}
              target="_blank">
              <h2 className="text-lg text-end flex bg-[#f1c88f] text-indigo-800 border p-3 rounded-full hover:shadow-indigo-400 shadow-lg cursor-pointer">
                {getName as string}
              </h2>
            </Link>
          </div>
              ...
              <tbody className="max-md:text-sm text-black">
                {[...winners].reverse().map((win, index) => (
                  <tr key={index} className="hover:opacity-80">
                    ...
                    <td className="border px-4 py-2 text-center">
                      <Link
                        href={`https://scrollscan.com/address/${win.rewardToken}`}
                        target="_blank"
