...
import ProgOCWNew from "./ProgressNew";
...
export const OCWChristmas = () => {
  ...
  const { data: getTotalEntries, refetch } = useReadContract({
    abi: NY_CONTRACT_ABI,
    address: NY_DAILY_CONTRACT_ADDRESS,
  ...
  //   : ((totalEntriesNumber * amountCostNumber) / targetPrizeAmountNumber) * 100;
    ...
    const progressValue = ((Number(currentPrize) / Number(prize)) * 100).toFixed(2);
      ...
      <motion.div
        ...
        <Block className="col-span-12 row-span-2 md:col-span-6 justify-between flex flex-col">
          <div className="font-bold">
            <p className="text-2xl sm:text-3xl md:text-4xl lg:text-7xl text-orange-200">
              New Year Daily Prize
            </p>
            <div className="border-t-2 mt-5 border-white/70" />
            <p className="mt-4 text-2xl sm:text-3xl md:text-4xl lg:text-5xl">
              {prize} ETH
            </p>
            <p className="text-orange-300 text-xl">Current Prize:</p>
            {parseFloat(currentPrize).toFixed(4)} ETH
          </div>
        ...
        <Block className="col-span-7 md:col-span-7 text-3xl flex flex-col items-center text-center justify-center">
          <p className="text-orange-400 text-base md:text-3xl">Total Ticket Sold</p>
          <ProgOCWNew percentage={progressValue} colour="orange" />
          {/* <Progress
            size="sm"
            radius="sm"
            classNames={{
